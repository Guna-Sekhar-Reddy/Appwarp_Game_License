...
//get matched rooms
function onGetMatchedRoomsDone(res) {
  console.log("Geting Matched Rooms");
  console.log(res);
  //console.log("boolean " + res.json.id == null);
  if (res.json.ids == "")
    //if no room is there then create one
    _warpclient.createRoom("GunaID", "LudoTournament", 2, getRoomProperties());
  //first parameter is ID not name -> we need to change
  else _warpclient.joinRoomWithProperties(getRoomProperties());
}
...
function onJoinRoomDone(res) {
  ...
  console.log(res);
  _warpclient.subscribeRoom(res.json.id);
}

//onSubscribe room
async function onSubscribeRoomDone(res) {
...
async function start() {
  let count = 0;
  while (count < 10) {
    _warpclient.connect(`Guna Sekhar ${count} `, "");
    await sleep(3000);
    count++;
  }
}
